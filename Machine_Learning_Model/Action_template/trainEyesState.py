...
def __add_landmark_to_df(landmark, df_values):
    """Helper function that adds a landmark to the list"""
...
def main(dirname, no_sequences, sequence_length):
    ...
    actions = np.array(["up", "down", "centre", "right", "left"])
                    ...
                    if landmarks is not None:
                     ...
                     # Apply wait logic
                        if count == 1:
                            ...
                            csv_path = os.path.join(DATA_PATH, '{}.csv'.format(action))

                            if not os.path.isfile(csv_path):

                                df_headers = []

                                __generate_df_headers(48, df_headers)
                            ...
                            with open(csv_path,'a', newline='') as csv_file:
                                writer_object = writer(csv_file)
                                writer_object.writerow(df_values)
...
if __name__ == "__main__":
    ...
    main( args.o, args.seqNo, args.seqLen)
